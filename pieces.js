...
class Piece {
    ...
    RemoveSameColorPiecesFromMoves(validMovesN) {
        ...
        let correctMoves = validMovesN.slice()
        for (let i = 0; i < validMovesN.length; i++) {
...
class King extends Piece {
    /* King Class
    Extends Piece class
    Adds letiable isInCheck and function IsKingInCheck */
    ...
    IsKingInCheck() {
        ...
        this.isInCheck = SqAttacked(this.squareOn120, this.color)
    }
}
...
for (let i = 0; i < pieceListWithTypes.length; i++) {
    for (let m = 0; m < pieceListWithTypes[i].length; m++) {
