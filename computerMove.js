...
function GenerateMoves(maxPlayer) {
    ...
    for (let v = 0; v < pieceList.length; v++) {
        ...
        if (pieceList[v].color == isWhite) {
            pieceList[v].validMoves = GetPieceMove(pieceList[v])
            boardEnPassant = possibleEnPassant
...
function MiniMax(depth, alpha, beta, maxPlayer) {
    ...
    if (depth == 0) {
        // console.log("Entering Quiescence: alpha=", alpha, "beta=", beta, "maxPlayer=", maxPlayer);
        let score = Quiescence(alpha, beta, maxPlayer)
        // let score = GetScoreBoard(boardRep120)
...
function Quiescence(alpha, beta, maxPlayer, level = 0) { // search only captures until reach stable position
    let boardScore = GetScoreBoard();
    // let quiesceScore = maxPlayer ? boardScore : -boardScore;
    ...
    // let oldBoardRep = boardRep120.slice()
    // let score = 0
    // let currBoardScore = 0
    // for (let i = 0; i < allCaptures.length; i++) {
    //     boardRep120 = allCaptures[i][0].slice()
    //     // currBoardScore = allCaptures[i][1] 
    //     // let playerScore = maxPlayer ? currBoardScore : -currBoardScore;
    //     ResetPieces();

    //     // Get Quiescence and set score
    //     score = -Quiescence(-beta, -alpha, !maxPlayer, level + 1) // negate score
